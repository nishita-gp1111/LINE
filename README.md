...
5. `/admin/rich-menus` で画像、タップ動作、条件タグを登録します。条件一致時はその顧客だけへ紐付け、条件がなくなると元のユーザー別メニューへ戻します。アンケートに完了後メニューを指定すると、完了者には即時、未完了者にも開始30分後（1〜1440分で設定可能）に同じメニューをユーザー単位で表示します。新規メニューはトーク画面で自動展開し、旧設定のメニューは「自動表示を修復」から画像とボタンを保ったまま再作成・対象者へ再紐付けできます。デフォルトリッチメニュー変更APIはサーバー側で常に拒否します。
