...
`INBOUND_EMAIL_NOTIFICATIONS_ENABLED=true` の環境では、顧客から新しいLINEメッセージを受信したあと、Resend経由で運用担当者へメール通知します。通知には顧客表示名、短い本文プレビュー、受信日時、対象会話を開くCRMリンクを含みます。WebhookのHTTP 200をメール送信で待たせず、通知失敗も正常なLINE受信を失敗へ変更しません。

同じ顧客から連続して届いたメッセージは、既存の受信履歴を使って既定10分間まとめます。LINEの再送は`webhookEventId`とmessage ID、メール事業者への再試行はmessage ID由来の冪等キーで重複を防ぎます。通知本文やメールアドレス、API keyはログへ出力しません。

本番では次のサーバー専用変数を設定します。送信元ドメインをResendで検証し、他の変数をすべて設定した最後に有効化してください。
...
RESEND_API_KEY= # Vercelのsecretにだけ保存
```
...
### Minimum Production Launchの7機能

初回の限定運用では、次の7機能をローンチ対象にします。

1. `/admin/contacts/[id]` で顧客へタグを付与・解除します。付与元はmanual / surveyごとに保持し、最後の付与元が解除された時だけタグなしになります。
2. `/admin/surveys` で1つの有効なアンケートを「友だち追加時」に指定します。follow webhook直後に挨拶カード、質問カードを順番に送信します。
...
5. `/admin/rich-menus` で画像、タップ動作、条件タグを登録します。基本メニューは友だち全員へ表示し、条件一致時はその顧客だけタグ別メニューへ切り替えます。条件がなくなると基本メニューへ戻ります。アンケートに完了後メニューを指定すると、完了者には即時、未完了者にも開始30分後（1〜1440分で設定可能）に同じメニューをユーザー単位で表示します。
6. `/admin/campaigns` で複数タグを選び、そのタグをすべて持つ顧客（AND条件）へ即時配信します。対象人数と除外人数を再計算し、最終確認に`配信する`と入力した場合だけ、重複を除外して500名以下のmulticast batchへ分割します。予約配信や全員broadcastは使用しません。
7. `/admin/acquisition` で「面談から流入」「アンケート経由」の友だち追加URLをコピーします。Googleフォームなど外部ブラウザーでは専用案内ページを表示し、LIFFで友だち追加、LINE本人確認、友だち状態確認を行って、メッセージ送信なしで同名タグを自動作成・付与します。同じ経路を再度開いてもタグを二重付与しません。LIFF未設定・未対応環境では、入力済み経路メッセージ方式へ安全にフォールバックします。

### 流入経路付き友だち追加URL

Productionでは次の固定URLを使用します。

- 面談から流入: `https://line-gp-1111.vercel.app/add/meeting`
- アンケート経由: `https://line-gp-1111.vercel.app/add/survey`

LINE Messaging APIの通常のfollowイベントには、どの友だち追加URLを経由したかを個人単位で示す値がありません。そのため各URLは、公式のLIFF URLへ経路パラメーターを付けてLINEを開きます。LIFF初期化完了後にだけ経路を読み取り、`requestFriendship()`と`getFriendship()`で友だち追加を案内・確認します。サーバーはブラウザーから受け取ったID tokenとaccess tokenをLINE APIで検証し、LINE Login profile、友だち状態、Messaging API profileのUser IDが一致した場合だけcontactと経路タグを保存します。LINE Login channelとMessaging API channelが同じProviderでない場合は登録を拒否します。PC幅のQRコードも同じLIFF経路URLを使用します。

トークン、LINE User ID、秘密値はレスポンス・画面・ログへ出しません。タグ付与は`source_type=system`と`source_id=acquisition:<slug>`の組み合わせで冪等です。LIFFが未設定、LINEアプリが`requestFriendship()`未対応、またはLINE連携確認に失敗した場合だけ、既存の友だち追加＋入力済み経路メッセージへフォールバックします。フォールバックメッセージはInboxへ保存しますが、新着メール通知の対象からは除外します。
...
- `LINE_RECIPIENT_MODE`: 既定値 `controlled`。Productionで明示的に`all_followers`へ切り替えた場合だけ、フォロワーへの個別送信・個別リッチメニュー変更を許可
- `ADMIN_EMAIL_ALLOWLIST`: 管理者メールアドレスのカンマ区切り。Milestone 0では表示・認証制御に未使用
- `INBOUND_EMAIL_NOTIFICATIONS_ENABLED`: 新着LINEのメール通知。既定値`false`
- `INBOUND_EMAIL_NOTIFICATION_RECIPIENTS`: 通知先メールアドレスのカンマ区切り
- `INBOUND_EMAIL_NOTIFICATION_FROM`: Resendで検証済みドメインの送信元メールアドレス
- `INBOUND_EMAIL_NOTIFICATION_COOLDOWN_MINUTES`: 同じ顧客の連続通知をまとめる時間。既定値10分
- `RESEND_API_KEY`: メール送信専用のサーバーsecret。ブラウザ・ログ・Gitへ出さない
...
#### 流入タグ用LINE Login / LIFF設定

1. 現在のMessaging API channelと**同じProvider**に、Web appのLINE Login channelを作成します。Providerが異なると同じ利用者でもUser IDが変わるため、サーバーは登録を拒否します。
2. LINE Login channelの「友だち追加オプション」で、`GP PRモニター窓口`をリンクします。
3. LIFF appを追加し、Sizeを`Full`、Endpoint URLを`https://line-gp-1111.vercel.app/liff/acquisition`、Scopesを`openid`と`profile`にします。LIFF URLへ付けた`source`は`liff.init()`完了後に読み取ります。
4. Vercel ProductionへLIFF IDを`NEXT_PUBLIC_LIFF_ID`、LINE Login Channel IDを`LINE_LOGIN_CHANNEL_ID`として設定します。`LINE_LOGIN_CHANNEL_SECRET`を設定する場合もサーバー専用とし、画面・ログ・Gitへ出しません。
5. Productionを再デプロイし、`GET /api/health`の`configured.liffId`、`configured.lineLoginChannelId`、`configured.acquisitionAutoTagging`がすべて`true`であることを確認します。
6. `/admin/acquisition`が「友だち追加後に自動反映」と表示された後、固定URLを配布します。未設定時は同じ固定URLのまま予備メッセージ方式で動作します。

LIFFの自動流入タグ機能にDB migrationは追加しません。既存のcontact upsert、tags、contact tag assignmentの冪等制約を使用します。
