...
7. `/admin/acquisition` で「面談から流入」「アンケート経由」の友だち追加URLをコピーします。追加後に入力済みの経路登録メッセージを1回送信すると、署名検証済みWebhookが同名タグを自動作成・付与します。同じ経路メッセージの再送ではタグを二重付与しません。
...
LINE Messaging APIの通常のfollowイベントには、どの友だち追加URLを経由したかを個人単位で示す値がありません。そのため各URLはLINEのトーク画面を開き、経路ごとの文面を入力済みにします。利用者が友だち追加後にその文面を1回送信した時点で、顧客と流入経路を決定的に紐付けます。経路登録メッセージはInboxへ保存しますが、新着メール通知の対象からは除外します。
