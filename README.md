...
Live modeでは `LINE_MANUAL_SEND_ENABLED`、`LINE_AUTOMATION_SEND_ENABLED`、`LINE_RICH_MENU_MUTATION_ENABLED` を運用対象環境だけで有効にし、`SURVEY_POSTBACK_TOKEN_SECRET` に32文字以上のランダム値を設定します。実LINEへの送信とユーザー別リッチメニュー変更は`LINE_TEST_USER_IDS`の完全一致、またはraw IDを保存しない`LINE_TEST_USER_HASHES`のSHA-256一致でサーバー側制限し、未設定時は全件を拒否します。ProductionではSho本人1件だけを許可し、0件または複数件なら一致する相手にも送信しません。

管理者専用の`GET /api/line/recipient-candidates`は、LINE follower ID APIが利用可能なアカウントに限り、候補の表示名とLINE user IDのSHA-256だけを返します。raw user ID、Channel Access Token、profile本文はレスポンスやログへ出しません。
