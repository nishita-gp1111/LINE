...
### Minimum Internal Launchの4機能

初回の内部運用では、次の4機能を管理画面から利用できます。

1. `/admin/contacts/[id]` で顧客へタグを付与・解除します。付与元はmanual / surveyごとに保持し、最後の付与元が解除された時だけタグなしになります。
2. `/admin/surveys` で選択肢ごとの付与タグと送信先顧客を設定します。1つのアンケートを友だち追加時の自動送信にも指定できます。回答は署名付きpostbackで受信し、同じ質問の再送は1件へ集約します。
3. `/admin/automations` で「タグ付与直後に1通送信」を作成して有効化します。同じタグには1つの有効設定だけを許可し、同じタグ付与の再処理では送信履歴を再利用します。
4. `/admin/rich-menus` で画像、タップ動作、条件タグを登録します。条件一致時はその顧客だけへ紐付け、条件がなくなると元のユーザー別メニューへ戻します。デフォルトリッチメニューは変更しません。

Live modeでは `LINE_MANUAL_SEND_ENABLED`、`LINE_AUTOMATION_SEND_ENABLED`、`LINE_RICH_MENU_MUTATION_ENABLED` を運用対象環境だけで有効にし、`SURVEY_POSTBACK_TOKEN_SECRET` に十分長いランダム値を設定します。`LINE_TEST_USER_IDS`を設定した場合、送信とユーザー別リッチメニュー変更は列挙したLINE user IDだけに制限されます。空の場合も、認証中organization内で画面から選択した顧客だけが対象です。一斉配信、予約配信、自動返信は別のflagであり、この4機能の有効化ではONになりません。
...
3. `supabase/migrations` をファイル名順にSupabase CLIで適用します。Minimum Internal Launchでは `20260714012000_minimum_internal_launch_follow_survey.sql` まで必要です。
