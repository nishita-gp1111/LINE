...
5. `/admin/rich-menus` で画像、タップ動作、条件タグを登録します。条件一致時はその顧客だけへ紐付け、条件がなくなると元のユーザー別メニューへ戻します。アンケートに完了後メニューを指定すると、完了者には即時、未完了者にも開始30分後（1〜1440分で設定可能）に同じメニューをユーザー単位で表示します。デフォルトリッチメニュー変更APIはサーバー側で常に拒否します。
...
`LINE_BULK_SEND_ENABLED`、`LINE_SCHEDULED_SEND_ENABLED`、`LINE_AUTO_REPLY_ENABLED`、`LINE_MEDIA_SEND_ENABLED`はfalseのままにします。一斉配信、予約配信、汎用の時間差配信、高度な分析、バックアップ自動化はMinimum Production Launchの合否には含めません。アンケート未完了者へのメニュー表示だけは、Supabase Cronから`/api/cron/dispatch`を1分ごとに呼び出して実行します。
