...
7. `/admin/acquisition` で「面談から流入」「アンケート経由」の友だち追加URLをコピーします。Googleフォームなど外部ブラウザーでは専用案内ページを表示し、利用者が明示的にLINEを開きます。追加後に入力済みの経路登録メッセージを1回送信すると、署名検証済みWebhookが同名タグを自動作成・付与します。同じ経路メッセージの再送ではタグを二重付与しません。
