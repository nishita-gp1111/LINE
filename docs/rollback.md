...
Rich menuは直前のAPI-managed default IDへ戻します。DB migrationは既存migrationを書き換えず、適用前backupとSupabaseの手動rollback計画を人間が確認します。secret漏えい時はLINE token/Channel Secret/CRON_SECRETをrotateし、ログを調査します。
