      ...
      <div className="mx-auto max-w-7xl">
        <section className="overflow-hidden rounded-2xl bg-gradient-to-br from-[#174f43] via-[#1d725f] to-[#17977b] text-white shadow-lg">
          <div className="grid gap-8 px-6 py-8 sm:px-9 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-center lg:py-10">
            <div><span className="rounded-full bg-white/15 px-3 py-1.5 text-[10px] font-black tracking-wide">MINIMUM PRODUCTION LAUNCH</span><h1 className="mt-4 text-3xl font-black tracking-tight sm:text-4xl">今日の顧客対応を、ここから。</h1><p className="mt-3 max-w-2xl text-sm leading-7 text-white/75">顧客との1対1トークを中心に、タグ・アンケート・即時メッセージ・個別リッチメニュー・タグ配信を迷わず操作できます。</p><div className="mt-6 flex flex-wrap gap-3"><Link href="/admin/inbox" className="focus-ring rounded-xl bg-white px-5 py-3 text-sm font-black text-emerald-800 shadow-sm">1対1トークを開く</Link><Link href="/admin/campaigns" className="focus-ring rounded-xl border border-white/30 bg-white/10 px-5 py-3 text-sm font-black text-white">タグ配信を開く</Link></div></div>
        ...
        <div className="mt-8 flex items-end justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-emerald-700">Quick start</p><h2 className="mt-1 text-2xl font-black">5つの運用メニュー</h2><p className="mt-1 text-sm text-ink/45">必要な機能をここから選べます。</p></div><Link href="/admin/settings/line" className="hidden text-xs font-black text-moss hover:underline sm:block">LINE接続状態を確認 →</Link></div>
