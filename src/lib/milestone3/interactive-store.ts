...
export function deactivateScenario(id: string): Scenario { const item = scenarios.find((scenario) => scenario.id === id); if (!item) throw new Error("scenario not found"); item.status = "paused"; return item; }
